 ...
 * @LastEditTime: 2021-03-06 16:04:01
-->
<template>
  <div>
    <div class="topBox">
    ...
    <fix-table :data="list" border v-loading="loading" element-loading-spinner="el-icon-loading"
      ...
      <!--      <el-table-column prop="user_template_id" label="模板ID"></el-table-column>-->
      ...
      <el-table-column label="小程序">
        <template slot-scope="scope">
          ...
          <span v-else-if="scope.row.type === '6'">超级直播</span>
          <span v-else>--</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="模板ID" prop="user_template_id" width="420"></el-table-column>
      <!--      <el-table-column align="center" label="推送时间" prop="send_time" width="100"></el-table-column>-->
    ...
    <el-dialog custom-class="my-dialog" :visible.sync="showaddtype" title="编辑推送" width="800px">
      <el-form :model="formData" label-width="80px" size="small">
        ...
        <el-form-item label="内容" label-width="100px">
          <el-input size="medium"  :rows="6" autocomplete="off" placeholder="推送关键词" type="textarea" clearable
...
export default {
  ...
  methods: {
    ...
    loadData () {
      this.loading = true
      this.getRequest('/XcxTemplatePush/getList', {
    ...
    editPost () {
      ...
      this.postRequest('/XcxTemplatePush/editPost', option).then(data => {
    ...
    sendPost () {
      this.postRequest('/XcxTemplatePush/sendPost', this.formData2).then(data => {
        if (data.code === 200) {
          this.$message.success(data.msg)
          this.showSend = false
        } else {
