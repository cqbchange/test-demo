    ...
    <div>
        ...
        <div class="table_box">
            <fix-table v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
                ...
                <el-table-column fixed prop="username" label="渠道名称" align="center" header-align="center" />
    ...
    export default {
        ...
        methods: {
            ...
            loadData() {
                ...
                this.getRequest('/Settlement/list', this.dataForm).then(
                    resp => {
                        this.loading = false
            ...
            editKp(row, state) {
                if (state == 3) {
                    this.$confirm('此操作将确认已支付发票数据, 是否继续?', '提示', {
                        ...
                        this.postRequest('/Settlement/paySuccess', {
                            id: row.id
                        }).then(resp => {
                            if (resp && resp.code === 200) {
                                this.$message({
                    ...
                    this.$confirm('此操作将确认已支付发票数据, 是否继续?', '提示', {
                        ...
                        this.postRequest('/Settlement/invoiceRefuseAgain', {
                            id: row.id
                        }).then(resp => {
                            if (resp && resp.code === 200) {
                                this.$message({
                    ...
                    this.$prompt('请输入异常理由', '提示', {
                        ...
                        if (value !== null) {
                            this.postRequest('/Settlement/invoiceRefuse', {
                                id: row.id,
                                refuse: value
                            }).then(resp => {
                                if (resp && resp.code === 200) {
                    ...
                    this.$confirm('此操作将确认发票数据, 是否继续?', '提示', {
                        ...
                        this.postRequest('/Settlement/invoiceConfirm', {
                            id: row.id
                        }).then(resp => {
                            if (resp && resp.code === 200) {
                                this.$message({
            ...
            editMoney(row) {
                this.$prompt('请输入更正金额', '提示', {
                    ...
                    if (value !== null) {
                        this.postRequest('/Settlement/editMoney', {
                            id: row.id,
