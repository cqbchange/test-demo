 ...
 * @LastEditTime: 2021-02-22 17:00:17
-->
<template>
  <div>
    <div class="topBox">
    ...
    <div class="table_box">
      ...
      >
        ...
        />
        <el-table-column
          prop="limit_click_amount"
          label="每日限制点击数"
          align="center"
        />
        <el-table-column
          prop="today_click_amount"
          label="今日点击数"
          align="center"
        />
        <el-table-column
    ...
    >
      <el-form :model="infoForm" size="medium">
        <el-row :gutter="20" type="flex">
          <el-col :span="24">
            <el-form-item label="主播ID：" label-width="135px">
              <el-input
                @paste.native="paste"
                v-model="infoForm.anchor_id"
                autocomplete="off"
                placeholder="请填写主播ID或复制链接"
                clearable
              />
            </el-form-item>
            <el-form-item label="每日限制点击数：" label-width="135px">
              <el-input
                v-model="infoForm.limit_click_amount"
                autocomplete="off"
                placeholder="请填写每日限制点击数"
                clearable
              />
            </el-form-item>
            <el-form-item label="累计限制数量：" label-width="135px">
              <el-input
                v-model="infoForm.limit_amount"
                autocomplete="off"
                placeholder="请填写累计限制数量"
                clearable
              />
            </el-form-item>
            <el-form-item label="状态：" label-width="135px">
              <el-radio v-model="infoForm.status" label="0">关闭</el-radio>
              <el-radio v-model="infoForm.status" label="1">开启</el-radio>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      this.loading = true;
      this.getRequest("/AnchorClick/getlist", this.dataForm).then((resp) => {
        this.loading = false;
    ...
    editData(row) {
      ...
      this.infoForm.limit_click_amount = row.limit_click_amount;
      this.infoForm.limit_amount = row.limit_amount;
    ...
    //   this.dialogVisible = false;
      ...
      let url = this.isadd ? "/AnchorClick/postadd" : "/AnchorClick/postupdate";

      this.postRequest(url, this.infoForm).then(
        (resp) => {
          if (resp && resp.code === 200) {
            this.$message({
              message: resp.msg,
    ...
    //删除
    delData(row) {
      this.$confirm("确认删除这条数据？")
        .then((_) => {
          this.postRequest("/AnchorClick/del", { id: row.id }).then((resp) => {
            if (resp && resp.code === 200) {
              this.$message({
                message: resp.msg,
                type: "success",
    ...
    discernFun(e) {
      ...
      // this.discern = str
      couldChange == true
        ? this.$set(this.dataForm, "keyword", str) && (couldChange = false)
        : "";
    },
  },
  computed: {},
